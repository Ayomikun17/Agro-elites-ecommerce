  ...
  return (
    <nav className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Reduced Height: h-16 for mobile, h-20 for desktop */}
      ...
      {/* MOBILE MENU */}
      <div className={`md:hidden bg-white transition-all duration-300 ${isOpen ? 'max-h-screen border-t' : 'max-h-0 overflow-hidden'}`}>
        <div className="p-6 space-y-4">
          {navLinks.map((link) => (
            <a key={link.name} href={link.href} className="block text-agro-charcoal font-bold text-lg">{link.name}</a>
          ))}
