...
// Manual SVG Icons for UI Elements
const Icons = {
...
const ProductsPage = () => {
  ...
  const calculateDiscount = (curr, orig) => {
    if (!orig) return null;
    const current = parseFloat(curr.replace(/[^\d.]/g, ''));
    const original = parseFloat(orig.replace(/[^\d.]/g, ''));
    if (isNaN(current) || isNaN(original)) return null;
    return Math.round(((original - current) / original) * 100);
  };

  const categoryData = [
    { name: 'All', image: 'https://images.unsplash.com/photo-1542838132-92c53300491e?q=80&w=200', bgColor: 'bg-[#F1F8E9]', desc: 'View all' },
    { name: 'Aquaculture', image: Aqualculture, bgColor: 'bg-[#E0F7FA]', desc: 'Catfish & Juveniles' },
    { name: 'Poultry', image: Poultry, bgColor: 'bg-[#FFF3E0]', desc: 'Dressed Chicken' },
    { name: 'Protein', image: Protein, bgColor: 'bg-[#FFFDE7]', desc: 'Goat & Beef' },
    { name: 'Crops', image: Maize, bgColor: 'bg-[#F3E5F5]', desc: 'Maize & Seasonal' },
    { name: 'Feeds', image: Feed, bgColor: 'bg-[#E8EAF6]', desc: 'Formulations' }
  ];
  ...
  const products = [
    ...
    },
    {
      ...
      fullDesc: "Raised under clean nursery conditions, our 8 cm juveniles give farmers the uniformity needed for growth tracking and FCR performance.",
    ...
    {
      id: 5,
      name: "Premium Dressed Organic Chicken",
      price: "₦6,000 per kg",
      originalPrice: "₦8,000",
      category: "Poultry",
      image: FrozenChicken,
      ...
      fullDesc: "Our dressed chicken is processed in a clean environment, quickly chilled and frozen to lock in flavour and safety.",
    ...
    },
    {
      ...
      fullDesc: "We formulate feeds for catfish, broilers, layers, and turkeys to match your production goals.",
      ...
      <div key={product.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-xl transition-all group flex flex-col h-full border border-gray-100">
        <div className="h-52 overflow-hidden relative">
          {product.originalPrice && (
            <>
              {/* Top Left: Promo Label */}
              <div className="absolute top-4 left-4 z-10 bg-red-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase shadow-lg animate-pulse">
                Promo Discount
              </div>
              {/* Bottom Right: Percentage */}
              <div className="absolute bottom-4 right-4 z-10 bg-agro-green text-white text-[12px] font-black px-3 py-1 rounded-xl shadow-lg">
                {discountPercent}% OFF
              </div>
            </>
          )}
        ...
        <div className="p-6 flex flex-col flex-grow">
          ...
          <div className="flex items-baseline gap-2 mb-3">
            <p className="text-agro-green font-black text-lg">{product.price}</p>
            {product.originalPrice && (
              <p className="text-gray-400 line-through text-sm font-bold">{product.originalPrice}</p>
            )}
          </div>
    ...
    <div id="products" className="bg-gray-50 min-h-screen py-16 px-6 scroll-mt-20">
      <div className="max-w-7xl mx-auto">
            ...
            {categoryData.map((cat) => (
              <button
                ...
                className={`flex items-center p-3 rounded-[2rem] transition-all border-2 text-left group h-full overflow-hidden ${
                  activeCategory === cat.name ? 'border-agro-green shadow-lg scale-105 ' + cat.bgColor : 'border-transparent bg-white hover:bg-gray-50 shadow-sm'
                }`}
              >
        ...
        <div className="space-y-24">
          {(activeCategory === 'All' ? categoryData.filter(c => c.name !== 'All') : categoryData.filter(c => c.name === activeCategory)).map(cat => {
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
             <div className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[3.5rem] shadow-2xl relative animate-in zoom-in-95 duration-300">
                ...
                <div className="grid md:grid-cols-2 gap-4 p-4 md:p-8">
                   <div className="h-[300px] md:h-full overflow-hidden rounded-[2.5rem] relative">
                      {selectedProduct.originalPrice && (
                         <div className="absolute top-6 left-6 z-10 bg-red-600 text-white text-[12px] font-black px-4 py-1.5 rounded-full uppercase shadow-xl animate-pulse">
                           Promo Discount (-{calculateDiscount(selectedProduct.price, selectedProduct.originalPrice)}%)
                         </div>
                      )}
                   ...
                   <div className="p-6 md:p-8 space-y-8">
                      <div>
                        ...
                        <div className="flex items-baseline gap-4 mt-2">
                          <p className="text-3xl font-black text-agro-yellow">{selectedProduct.price}</p>
                          {selectedProduct.originalPrice && (
                            <p className="text-xl text-gray-400 line-through font-bold">{selectedProduct.originalPrice}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="pt-6 border-t border-gray-100">
                        <a href={`https://wa.me/2348035138049?text=Order: ${selectedProduct.name}`} className="block w-full bg-agro-yellow text-agro-charcoal py-5 rounded-[1.5rem] text-center font-black uppercase tracking-widest hover:bg-agro-green hover:text-white transition-all shadow-xl text-sm">
                          Order via WhatsApp
                        </a>
                      </div>
                   </div>
